...
- **审批管理**：管理员可审批游客的志愿者身份申请，也可审批志愿者对具体活动的参与申请。两个审批区块采用 `admin-grid` 两列格栅并排布局，统一使用 `.panel` 卡片包裹，样式一致、宽度对齐，便于演示完整的角色流转与分工流程。
...
前端位于 `frontend/index.html`（当前约 3145 行），采用 Vue 3 单页应用（通过 CDN 引入 `vue.global.prod.js`），无需构建步骤即可运行。
